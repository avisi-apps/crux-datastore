(ns avisi.crux.fixtures
  ...
  (:import
    [com.google.cloud.datastore Datastore DatastoreOptions]
...
(defn with-datastore-emulator [f]
  (let [datastore-helper ^LocalDatastoreHelper (LocalDatastoreHelper/create (int 8081))
    ...
    (binding [*datastore* (.getService options)
      ...
      (try (.start datastore-helper) (f) (finally (.stop datastore-helper))))))

(def ^:dynamic ^ICruxAPI *api*)
(def ^:dynamic *opts* nil)

(defn with-opts [opts f] (binding [*opts* (merge *opts* opts)] (f)))
...
(defn with-clean-datastore [f] (try (f) (finally (.reset *datastore-helper*))))

(defn with-node [f] (with-open [node (crux/start-node *opts*)] (binding [*api* node] (f))))

(defn submit+await-tx ([tx-ops] (submit+await-tx *api* tx-ops))
  ([api tx-ops]
